...
This approach allows us to define the \textit{execute} operation without
introducing a program counter.


% Jump
We then split two distinct usages of the \textit{jump} operation:
\textit{enter} and \textit{exit}.
Since Wasm supports structured control flow, the \textit{jump} operation is
used either for entering or exiting control structures such as a function body
and a block of instructions.
% Enter
We define the \textit{enter} as the operation of pushing the context associated
with the jump onto the interleaved stack and the instructions within
the structure onto the instruction stack.
On the contrary, we define \textit{exit} as the operation of popping the
corresponding context and instructions.


% State transition
To express the \textit{enter} and \textit{exit} operation formally, we model
...
The \textit{enter} operation can be expressed as the following state transition:
\[
...
% TODO: Fix end instruction in the 5.3

% End
Note that we introduce an administrative instruction, \textbf{end}, to mark the
end of the structure in the instruction stack.
The prose specification describes a behavior that must take place when the end
of a structure is reached without control transfer.
\cref{fig:exiting-label} illustrates this behavior for a function.
While not a Wasm instruction itself, it is presented in the same format as Wasm
instructions.
...
Similarly, the \textit{exit} operation can be described as a transition of Wasm
state.
...
The \textit{exit} operation can be represented as the following state transition:
\[
