...
% state
An AL state consists of sequence of algorithm, a Wasm state, and a
continuation.
% algorithm
...
usage.
% Wasm state
Wasm state consists of a Wasm value stack, an Wasm instruction stack, and a
store.
% Wasm value & instruction
A Wasm value and a Wasm instruction are expressed in values, especially in the
form of $\casev$.
% store
A store is expressed in a sequence of a field name and an AL value pairs which
models records.
% continuation
A continuation is \mt{}; \call{} with a variable, a function name, arguments,
and a continuation; \wasm with contexts and a continuation; \exe with a Wasm
instruction and a continuation; \algo with AL instructions, an environment,
contexts, and a continuation; or \ret with a AL value and a continuation.
% env
...
instruction, executing Wasm instruction sequence, AL function call with let
binding, or replacing some mutable data structure.
% expression
...
% CallTopLevel
Given the algorithms $\seq a$ generated from the DSL and Wasm state $w$, a
...
Wasm instruction is popped from the instruction stack and executed until the
context sequence $\seq c$ is exhausted.
...
processes Wasm instructions, changing the continuation to \algo{}.
% Algo
