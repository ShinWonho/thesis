...
\newline \\
  ...
  wi = \getendinstr(v_1) \qquad
  wi_1 = \getendinstr(c_1) \quad ... \quad wi_n = \getendinstr(c_n) \\
  \hline
  \seq{a}, w, k \vdash \enteri ~ (e_1, e_2)
  \Rightarrow
  (
    (v_1 ~ \seq{wv}, \seq{v_2} ~ wi ~ wi_1 ~ ... ~ wi_n ~ \seq{wi}, sto),
    \wasm ~ (v_1 ~ c_1 ~ ... ~ c_n, \clearctx(k))
  ) \\
%
% PushCtx
\newline \\
...
\newline \\
  ...
  \hline
  \seq{a}, w, k \vdash \calli ~ (s_1, s_2, e_1 ~ ... ~ e_n) \Rightarrow
  (w, \call ~ (s_1, s_3, v_1 ~ ... ~ v_n, k)) \\
%
% Call-fname
\newline \\
  \mu = \getenv(k) \qquad
  ...
  w, \mu \vdash e_n \Rightarrow v_n \\
  \hline
  \seq{a}, w, k \vdash \calli ~ (s_1, s_2, \seq e) \Rightarrow
  (w, \call ~ (s_1, s_2, v_1 ~ ... ~ v_n, k)) \\
%
% Replace-frame
\newline \\
\text{NOTE: $e_1$ is either store or current frame} \\
  ...
  \hline
  \seq{a}, w, k \vdash \replacei ~ (e_1, p^+, e_2) \Rightarrow
  (\setcurframe(w, \casev ~ (\text{"FRAME"}, v_{11} ~ v_3)), k) \\
%
% Replace-store
\newline \\
  \mu = \getenv(k) \qquad
  ...
  \hline
  \seq{a}, w, k \vdash \replacei ~ (e_1, p^+, e_2)
...
% exit_cont-al
  &\exit_{K}(\algo ~ (\seq i, \mu, epsilon, k)) = \algo ~ (\seq i, \mu, \seq c, \exit(k)) \\
  &\exit_{K}(\algo ~ (\seq i, \mu, c ~ \seq c, k)) = \algo ~ (\seq i, \mu, \seq c, k) \\
%
% exit_wasm_instr-end
\newline \\
