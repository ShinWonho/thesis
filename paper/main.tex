...
\newcommand{\hatcode}{\textsuperscript{$\wedge$}}
...
% TODO: not approved!
\approvaldate{2024}{12}{???}

% @command refereedate 심사위원논문심사일
% @param   year,month,day 연,월,일 순으로 입력
\refereedate{2024}{12}{12}
...
\begin{document}
 ...
 %% Don't write english letters in the abstract in korean.
  ...
  \begin{abstract}
  WebAssembly (Wasm) is a low-level bytecode language that is safe, fast,
  portable, and compact, widely used as a compilation target for web
  applications.
  The Wasm specification is particularly rigorous, describing its semantics in
  two complementary forms: \textit{formal notation} and \textit{prose
  notation}.
  The demanding standardization process places a significant burden on
  specification authors, particularly in maintaining correctness.
  To mitigate this problem, we propose SpecTec, a framework for mechanizing
  Wasm specification.
  ...
  errors, and translate it into the prose specification with algorithmic
  specification language.
  This algorithmic language is designed with rigor to correctly specify Wasm
  semantics in the prose notation.
  One notable aspect of this language is that it is executable, meaning any
  Wasm specification written in this language serves as a Wasm interpreter
  program.
  ...
  \begin{Engkeyword}
  WebAssembly, executable specification, mechanized specification, language
