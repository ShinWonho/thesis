...
\red{TODO: explanation of context: in the background? or here?} \\
...
related to the control flow are omitted here.
...
\texttt{call} instruction followed by a \texttt{f32.const} instruction, which can be expressed in \cref{eq:call-1}.
...
With this \texttt{end} instruction, it seems that \spectecp{} can model the
WebAssembly control flow correctly.
However, the \red{interpreter} implemented with this model doesn't work well.
