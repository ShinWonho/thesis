...
\newline \\
  ...
  \hline
  (\seq{a}, w, \call ~ (str_0, str_1, \seq v, c)) \leadsto (\seq{a}, w, \al ~ (\seq i, \mu, \call ~ (str_0, str_1, \seq v, c))) \\
%
% Return
  \mu = \getenv(c) \quad c' = \setenv(\mu[str_0 \mapsto v]) \\
  \hline
  (\seq{a}, w, \ret ~ (v, \call ~ (str_0, str_1, \seq v, c)) \leadsto c' \\
%
% Al-empty
\newline \\
  \hline
  (\seq{a}, w, \al ~ (\epsilon, \mu, 0, c)) \leadsto (\seq{a}, w, c) \\
%
% Al-instr
\newline \\
  \seq{a}, w, \al ~ (\seq{i_1}, \mu, n, c) \vdash i_0 \Rightarrow (w', c') \\
  \hline
  (\seq{a}, w, \al ~ (i_0 ~ \seq{i_1}, \mu, n, c)) \leadsto (\seq{a}, w', c') \\
%
% Wasm-zero
\newline \\
  \hline
  (\seq{a}, w, \wasm ~ (0, c)) \leadsto (\seq{a}, w, c) \\
%
% Wasm-value
\newline \\
  n > 0 \quad \iswasmvalueinstr(wi) \\
  \hline
  (\seq{a}, (wi ~ \seq{wi'}, \seq{wv}, sto), \wasm ~ (n, c))
  \leadsto
  (\seq{a}, (\seq{wi'}, wi ~ \seq{wv}, sto), \wasm ~ (n, c)) \\
%
% Wasm-instr
\newline \\
  ...
  (\seq{a}, (wi ~ \seq{wi'}, \seq{wv}, sto), \wasm ~ (n, c))
  \leadsto
  (\seq{a}, (\seq{wi'}, \seq{wv}, sto), \al ~ (\seq i, \seq v, \wasm ~ (n, c))) \\
%
% Execute-value
\newline \\
  \iswasmvalueinstr(wi') \\
  \hline
  (\seq{a}, (\seq{wi}, \seq{wv}, sto), \exe ~ (wi', c))
  \leadsto
  (\seq{a}, (\seq{wi}, wi' ~ \seq{wv}, sto), c) \\
%
% Execute-instr
\newline \\
  \neg \iswasmvalueinstr(wi') \\
  ...
  \mu = \assign([\text{"s"} \mapsto \storev], \seq e, \seq v) \\
  \hline
  (\seq{a}, w, \exe ~ (wi', c)) \leadsto (\seq{a}, w, \al ~ (\seq i, \seq v, c)) \\
\end{gather*}
...
\begin{gather*}
  ...
  \seq{a}, ~ w, ~ c \vdash i \Rightarrow w, ~ c \\
%
% If-true
\newline \\
  ...
  \hline
  \seq{a}, w, c \vdash \ifi ~ (e, \seq{i_1}, \seq{i_2}) \Rightarrow (w, c') \\
%
% If-false
\newline \\
  \mu = \getenv(c) \quad w, \mu \vdash e \Rightarrow v \quad
...
\newline \\
  \hline
  \seq{a}, w, c \vdash \eitheri ~ (\seq{i_1}, \seq{i_2}) \Rightarrow (w, \prependinstr(c, \seq{i_1})) \\
%
% Either-2
\newline \\
  \hline
  \seq{a}, w, c \vdash \eitheri ~ (\seq{i_1}, \seq{i_2}) \Rightarrow (w, \prependinstr(c, \seq{i_2})) \\
%
% Enter
\newline \\
  \mu = \getenv(c) \quad
  ...
  \hline
  \seq{a}, (\seq{wv}, \seq{wi}, sto), c \vdash \enteri ~ (e_1, e_2)
  \Rightarrow
...
\newline \\
  ...
  \hline
  \seq{a}, w, c \vdash \pushi ~ e \Rightarrow (\push_{WasmValue}(w, v), c) \\
%
% PopCtx
\newline \\
  \mu = \getenv(c) \quad
  ...
  \hline
  \seq{a}, (\seq{wv}, \seq{wi}, sto), c \vdash \popctxi ~ e
  \Rightarrow
...
\newline \\
  ...
  \mu' = \assign(\mu, e, wv) \\
  \hline
  \seq{a}, w, c \vdash \popi ~ e \Rightarrow (w', \setenv(c, \mu')) \\
%
% PopN
\newline \\
  \mu = \getenv(c) \quad
  ...
  \hline
  \seq{a}, w, c \vdash \popni ~ (e_1, e_2) \Rightarrow (w', \setenv(c, \mu')) \\
%
% PopAll
\newline \\
  \mu = \getenv(c) \quad
...
\newline \\
  \hline
  \seq{a}, w, \al~ (\seq{i}, \mu, n, c) \vdash \returnrulei \Rightarrow (w, c) \\
%
% Return-func
\newline \\
  w, \mu \vdash e \Rightarrow v \\
  \hline
  \seq{a}, w, \al ~ (\seq{i}, \mu, n, c) \vdash \returnfunci ~ e \Rightarrow (w, \ret ~ (v, c)) \\
%
% Execute
\newline \\
  \mu = \getenv(c) \quad
  w, \mu \vdash e \Rightarrow v \\
  \hline
  \seq{a}, w, c \vdash \executei ~ e \Rightarrow (w, \exe ~ (v, c)) \\
%
% ExecuteSeq
\newline \\
  \mu = \getenv(c) \quad
  ...
  \hline
  \seq{a}, w, c \vdash \executeseqi ~ e \Rightarrow (w, c') \\
%
% Call-fname
\newline \\
  \mu = \getenv(c) \quad
  ...
  \hline
  \seq{a}, w, c \vdash \calli ~ (str_0, str_1, \seq e) \Rightarrow (w, \call ~ (str_0, str_2, \seq v, c)) \\
%
% Call-fname
\newline \\
  \mu = \getenv(c) \quad
  ...
  \seq{v} = [ ~ v ~ | ~ e \leftarrow \seq{e}; ~ w, \mu \vdash e \Rightarrow v ~ ] \\
  \hline
  \seq{a}, w, c \vdash \calli ~ (str_0, str_1, \seq e) \Rightarrow (w, \call ~ (str_0, str_1, \seq v, c)) \\
%
% Replace-frame
\newline \\
\text{NOTE: $e_1$ is either store or current frame} \\
  ...
  \hline
  \seq{a}, w, c \vdash \replacei ~ (e_1, \seq{path}, e_2) \Rightarrow (\setcurframe(w, v_3), c) \\
%
% Replace-store
\newline \\
  ...
  \hline
  \seq{a}, (\seq{wv}, \seq{wi}, sto), c \vdash \replacei ~ (e_1, \seq{path}, e_2)
